...
def minxent_gradient2(q: np.ndarray, G: np.ndarray, eta: np.ndarray, lambda_: np.ndarray, maxiter: int):
    startTime = time.time()
    q = q.astype(float)
    G = G.astype(float)
    eta = eta.astype(float)
    ...
    while iter_general <= maxiter and norm(lambda_ - lambda_old) >= 1e-08:
        iter_general += 1
        ...
        while not (did_ascent and did_descent):
            ...
            if alpha < 1e-06 or alpha > 1e2:
                log("Leaving gradient descent due to high alpha value", lg.WARNING)
                break
